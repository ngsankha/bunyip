...
var newJob = function(callback) {
  ...
  client.multi()
    .set('bunyip::job::' + id, JSON.stringify({ status: 'pending' }))
...
var markForDeletion = function(id, filename, callback) {
  ...
  client.set('bunyip::job::' + id, JSON.stringify({ status: 'done', path: filename }), function(err, res) {
    if (err) {
...
var getJob = function(id, callback) {
  ...
  client.get('bunyip::job::' + id, function(err, reply) {
    if (err) {
